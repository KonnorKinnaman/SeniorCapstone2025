...
from ChatGPTVA import load_env_file, get_api_key, ask_chatgpt
from speech_rec import record_text, output_text
...
def main():
    ...
    first_msg = input("Press [S] to enter scan mode, or press [C] to use chat mode: ")
    ...
    if first_msg == "S":
        ...
        method = input("Choose input method (t)ext or (s)peech: ").lower()
        if method in ('', 't'):
            # Text input
            prompt = input("\nWhat would you like to ask ChatGPT?\n")
        elif method == 's':
            #add check for audio methods
                
            # Speech input
            try:
                print("Starting recording when you press 's'...")
                audio_data = record_text()  # Will record until 's' is pressed again
                prompt = audio_data
